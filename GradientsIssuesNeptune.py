...
def getBatchGradWgts(grads, wgts, lossVal, 
    ...
    for wgt, grad in zip(wgts, grads):
        ...
        ## Log in Neptune
        if npt_exp:
            npt_exp[f'monitoring/logs/MeanGrads{layerName.upper()}'].log(np.mean(grad.numpy()))   
            npt_exp[f'monitoring/logs/MeanWgtBatch{layerName.upper()}'].log(np.mean(wgt.numpy()))         
    ...
    for epoch in range(n_epochs):
        ...
        for step, (x_batch, y_batch) in enumerate(subData):
            ...
            if step == 5:
                ...
                if npt_exp:
                    npt_exp['monitoring/logs/BatchLoss'].log(lossVal)   
