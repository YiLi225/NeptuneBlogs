'''
ImageSegmentationProject for Neptune new version
Mask R-CNN model image segmentation modeling and tuning 
'''
import os
import neptune.new as neptune  ## ! new version
# Use case for image segmentation 
# # Connect your script to Neptune
# project = neptune.init(api_token=os.getenv('NEPTUNE_API_TOKEN'),
#                        project_qualified_name='katyl/ImageSegmentationProjectNewVersion') ## 'YourUserName/YourProjectName'
...
# Root directory of the project (new version)
ROOT_DIR = os.path.abspath(r"'Root directory of your project'")  
...
# Import COCO config
sys.path.append(os.path.join(ROOT_DIR, "samples/coco/"))  # !! To find local version (new version)
import coco 
...
### log text to Neptune new version 
...
config = CustomConfig()
## Log current config to Neptune new version
## npt_exp.send_text('Model Config Pars', str(config.to_dict()))
npt_exp['Model Config Pars'] = str(config.to_dict())
...
cur_image_path = 'YourImagePath'    ### !!! 'YourImagePath' new version
